...
model.addConstrs(((acu[i,t] * (x[i,t] + h[i,t]) * ec[i,r])/(365) <= 50000000 for i in Plantas for t in Tiempo for r in Region), name = "R2" ) 
...
# model.addConstrs((ii[r,t] == ii[r,t-1] + quicksum((x[i,t] + h[i,t]) for i in Plantas) for t in range(2, max(Tiempo) + 1) for r in Region), name = "R8.1")
# model.addConstrs((ii[r,1] == 0 + quicksum((x[i,1] + h[i,1]) for i in Plantas) for r in Region), name = "R8.2")
